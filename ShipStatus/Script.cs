...
{this.CloseAll(airlockDoors.blocks);}else{this.Lock(this.areClosed);this.Unlock(this.areOpen);}return true;}}public struct FillPct{public VRage.MyFixedPoint current;public VRage.MyFixedPoint max;public
string name;public FillPct(VRage.MyFixedPoint current,VRage.MyFixedPoint max,string name){this.current=current;this.max=max;this.name=name;}}public class CargoStatus:Runnable{public Program program;public
List<IMyTerminalBlock>cargoBlocks;public Dictionary<string,VRage.MyFixedPoint>cargoItemCounts;public Dictionary<long,FillPct>containerPercentages;public List<MyInventoryItem>inventoryItems;public System.Text.RegularExpressions.Regex
itemRegex;public System.Text.RegularExpressions.Regex ingotRegex;public System.Text.RegularExpressions.Regex oreRegex;public System.Text.RegularExpressions.Regex filterContainers;public VRage.MyFixedPoint
max;public VRage.MyFixedPoint vol;public Template template;public List<float>widths;public string itemText;public float pct;public CargoStatus(Program program,Template template=null){this.program=program;
this.template=template;this.itemRegex=Util.Regex(".*/");this.ingotRegex=Util.Regex("Ingot/");this.oreRegex=Util.Regex("Ore/(?!Ice)");this.widths=new List<float>(){0,0,0,0};this.cargoItemCounts=new Dictionary<string,
VRage.MyFixedPoint>();this.inventoryItems=new List<MyInventoryItem>();this.cargoBlocks=new List<IMyTerminalBlock>();this.containerPercentages=new Dictionary<long,FillPct>();this.itemText="";this.pct=0f;
this.Reset();}public void Reset(){this.Clear();this.filterContainers=Util.Regex(this.program.config.Get("cargoIgnore","$^"));if(this.program.config.Enabled("cargo")){this.RegisterTemplateVars();}}public
void Clear(){this.cargoItemCounts.Clear();this.inventoryItems.Clear();this.cargoBlocks.Clear();this.containerPercentages.Clear();}public void RegisterTemplateVars(){if(this.template==null){return;}this.template.Register("cargo.stored",
()=>$"{Util.FormatNumber(1000 * this.vol)} L");this.template.Register("cargo.cap",()=>$"{Util.FormatNumber(1000 * this.max)} L");this.template.Register("cargo.fullString",()=>this.Filled(this.vol,this.max));
this.template.Register("cargo.bar",(DrawingSurface ds,string text,DrawingSurface.Options options)=>this.CargoBar(ds,text,options,this.pct));this.template.Register("cargo.items",this.CargoItems);this.template.Register("cargo.item",
this.CargoItem);this.template.Register("cargo.containers",this.CargoContainerPercentages);this.template.Register("cargo.containerBars",this.CargoContainerBars);}public string Filled(VRage.MyFixedPoint
vol,VRage.MyFixedPoint max){string capFmt=Util.GetFormatNumberStr(1000*max);return$"{Util.FormatNumber(1000 * vol, capFmt)} / {Util.FormatNumber(1000 * max, capFmt)} L";}public void CargoContainerPercentages(DrawingSurface
ds,string text,DrawingSurface.Options options){if(this.containerPercentages.Count()==0){ds.Text(" ");return;}foreach(var item in this.containerPercentages){string filled=this.Filled(item.Value.current,
item.Value.max);float pct=(float)item.Value.current/(float)item.Value.max;ds.Text($"{item.Value.name}").SetCursor(ds.width,null).Text($"{filled} ({Util.PctString(pct)})",textAlignment:TextAlignment.RIGHT).Newline();
}ds.Newline(reverse:true);}public void CargoContainerBars(DrawingSurface ds,string text,DrawingSurface.Options options){if(this.containerPercentages.Count()==0){ds.Text(" ");return;}foreach(var item in
this.containerPercentages){string filled=this.Filled(item.Value.current,item.Value.max);float pct=(float)item.Value.current/(float)item.Value.max;DrawingSurface.Options barOpts=new DrawingSurface.Options();
barOpts.text=$"{item.Value.name} {filled} ({Util.PctString(pct)})";barOpts.pct=pct;barOpts.custom=options.custom;barOpts.textColour=options.textColour;this.CargoBar(ds,text,barOpts,pct);ds.Newline();}
ds.Newline(reverse:true);}public void CargoBar(DrawingSurface ds,string text,DrawingSurface.Options options,float pct){Color defaultText=options.textColour??ds.surface.ScriptForegroundColor;string colourName
=options.custom.Get("colourLow")??"dimgreen";options.textColour=DrawingSurface.StringToColour(options.custom.Get("textColourLow"))??defaultText;if(pct>0.85){colourName=options.custom.Get("colourHigh")
??"dimred";options.textColour=DrawingSurface.StringToColour(options.custom.Get("textColourHigh"))??defaultText;}else if(pct>0.60){colourName=options.custom.Get("colourMid")??"dimyellow";options.textColour
=DrawingSurface.StringToColour(options.custom.Get("textColourMid"))??defaultText;}options.pct=pct;options.fillColour=DrawingSurface.StringToColour(colourName);options.text=options.text??Util.PctString(pct);
ds.Bar(options);}public void CargoItem(DrawingSurface ds,string text,DrawingSurface.Options options){string name=options.custom.Get("name");VRage.MyFixedPoint itemCount;if(!this.cargoItemCounts.TryGetValue(name,
out itemCount)){itemCount=0;}ds.Text(Util.FormatNumber(itemCount),options);}public void CargoItems(DrawingSurface ds,string text,DrawingSurface.Options options){if(this.cargoItemCounts.Count()==0){ds.Text(" ");
return;}if(ds.width/(ds.charSizeInPx.X+1f)<40){foreach(var item in this.cargoItemCounts){var fmtd=Util.FormatNumber(item.Value);ds.Text($"{item.Key}").SetCursor(ds.width,null).Text(fmtd,textAlignment:
TextAlignment.RIGHT).Newline();}}else{this.widths[0]=0;this.widths[1]=ds.width/2-1.5f*ds.charSizeInPx.X;this.widths[2]=ds.width/2+1.5f*ds.charSizeInPx.X;this.widths[3]=ds.width;int i=0;foreach(var item
in this.cargoItemCounts){var fmtd=Util.FormatNumber(item.Value);ds.SetCursor(this.widths[(i++%4)],null).Text($"{item.Key}").SetCursor(this.widths[(i++%4)],null).Text(fmtd,textAlignment:TextAlignment.RIGHT);
if((i%4)==0||i>=this.cargoItemCounts.Count*2){ds.Newline();}}}ds.Newline(reverse:true);}public void GetBlock(IMyTerminalBlock block){if(block is IMyCargoContainer||block is IMyShipDrill||block is IMyShipConnector
||block is IMyAssembler||block is IMyRefinery){if(!this.filterContainers.Match(block.CustomName).Success){this.cargoBlocks.Add(block);}}}public void GotBLocks(){}public void Refresh(){if(this.cargoBlocks
==null){return;}this.cargoItemCounts.Clear();this.inventoryItems.Clear();this.containerPercentages.Clear();this.max=0;this.vol=0;this.pct=0f;string fullName="";string itemName="";foreach(var c in this.cargoBlocks)
{if(!Util.BlockValid(c)){continue;}var inv=c.GetInventory(0);if(inv.MaxVolume==0){continue;}this.containerPercentages.Add(c.EntityId,new FillPct(inv.CurrentVolume,inv.MaxVolume,c.CustomName));this.vol
...
=this.itemRegex.Replace(fullName,"");if(itemName=="Stone"){if(this.ingotRegex.IsMatch(fullName)){itemName="Gravel";}else{itemName="Stone";}}else if(this.ingotRegex.IsMatch(fullName)){itemName+=" Ingot";
}else if(this.oreRegex.IsMatch(fullName)){itemName+=" Ore";}var itemQty=this.inventoryItems[i].Amount;if(!this.cargoItemCounts.ContainsKey(itemName)){this.cargoItemCounts.Add(itemName,itemQty);}else{this.cargoItemCounts[itemName]
...
name){return this.settings.Get(name)=="true";}}public bool ParseCustomData(){MyIniParseResult result;if(!ini.TryParse(Me.CustomData,out result)){Echo($"Failed to parse config:\n{result}");return false;
}if(Me.CustomData==""){Me.CustomData=customDataInit;}config.Clear();config.customData=Me.CustomData;strings.Clear();ini.GetSections(strings);template.Reset();templates.Clear();config.Set("airlock","true");
string themeConfig="";if(ini.ContainsSection("global")){string setting="";List<MyIniKey>keys=new List<MyIniKey>();ini.GetKeys("global",keys);foreach(MyIniKey key in keys){if(ini.Get(key).TryGetString(out
...
(this.AnyGeneratorEnabled()){message=options.custom.Get("txtEnabled")??"Gas generation on";}ds.Text(message,options);});this.template.Register("gas.generationEnabledExtended",(DrawingSurface ds,string
text,DrawingSurface.Options options)=>{this.GasExtendedMessage(ds,text,options);});this.template.Register("gas.o2Bar",(DrawingSurface ds,string text,DrawingSurface.Options options)=>{this.GasBar((float)o2FillPct,
...
}}else if(block is IMyGasGenerator&&block.DefinitionDisplayNameText.ToString()!="Irrigation System"){this.program.log.Append($"gas gen: {block.DefinitionDisplayNameText.ToString()}\n");this.oxyGens.Add((IMyGasGenerator)block);
}}public void GotBLocks(){this.enableFillPct=Util.ParseFloat(this.program.config.Get("gasEnableFillPct"),-1f);this.disableFillPct=Util.ParseFloat(this.program.config.Get("gasDisableFillPct"),-1f);}public
void Refresh(){this.ClearTotals();this.GetGasLevels();}public void ClearTotals(){this.o2CurrentVolume=0f;this.o2MaxVolume=0f;this.o2FillPct=0f;this.o2TankCount=0;this.h2CurrentVolume=0f;this.h2MaxVolume
=0f;this.h2FillPct=0f;this.h2TankCount=0;}public void PrintTanks(List<IMyGasTank>tanks,DrawingSurface ds,DrawingSurface.Options options){if(tanks.Count==0){ds.Text("");return;}foreach(IMyGasTank tank in
tanks){double currentVolume=(tank.FilledRatio*tank.Capacity);ds.Text($@"{tank.CustomName}: {Util.FormatNumber(currentVolume)} / {Util.FormatNumber((double)tank.Capacity)} L ({Util.PctString(tank.FilledRatio)})",
...
??ds.surface.ScriptForegroundColor;ds.Bar(options);}public void SetGenerators(bool enabled){foreach(IMyGasGenerator genny in this.oxyGens){genny.Enabled=enabled;}}public bool AnyGeneratorEnabled(){foreach
(IMyGasGenerator genny in this.oxyGens){if(genny.Enabled){return true;}}return false;}public void GetGasLevels(){this.tankMap.Clear();foreach(IMyGasTank tank in this.o2Tanks){this.o2CurrentVolume+=(tank.FilledRatio
*tank.Capacity);this.o2MaxVolume+=tank.Capacity;this.o2TankCount++;if(!this.tankMap.ContainsKey(tank.CustomName)){this.tankMap.Add(tank.CustomName,tank.FilledRatio);}else{this.tankMap[tank.CustomName]
=this.tankMap[tank.CustomName]+tank.FilledRatio;}}foreach(IMyGasTank tank in this.h2Tanks){this.h2CurrentVolume+=(tank.FilledRatio*tank.Capacity);this.h2MaxVolume+=tank.Capacity;this.h2TankCount++;if(!this.tankMap.ContainsKey(tank.CustomName))
...
options){var current=Math.Ceiling(100*Math.Min(this.o2FillPct,this.h2FillPct));string message=options.custom.Get("txtDisabled")??$"Gas generation off";if(this.enableFillPct!=-1){message=message+$", enabled at {Math.Floor(100 * this.enableFillPct)}% (current: {current}%)";
}if(this.AnyGeneratorEnabled()){message=options.custom.Get("txtEnabled")??$"Gas generation on";if(this.disableFillPct!=-1){message=message+$", disabled at {Math.Ceiling(100 * this.disableFillPct)}% (current: {current}%)";
}}ds.Text(message,options);}}class BlockHealth:Runnable{public Program program;public Template template;public System.Text.RegularExpressions.Regex ignoreHealth;public List<IMyTerminalBlock>blocks;public
Dictionary<string,string>damaged;public Dictionary<long,IMyTerminalBlock>shownOnHud;public string status;public BlockHealth(Program program,Template template){this.program=program;this.template=template;
this.blocks=new List<IMyTerminalBlock>();this.damaged=new Dictionary<string,string>();this.shownOnHud=new Dictionary<long,IMyTerminalBlock>();this.Reset();}public void Reset(){this.Clear();this.shownOnHud.Clear();
...
this.template.Register("health.blocks",(DrawingSurface ds,string text,DrawingSurface.Options options)=>{if(this.damaged.Count()==0){ds.Text("");return;}foreach(KeyValuePair<string,string>block in this.damaged)
{ds.Text($"{block.Key} [{block.Value}]").Newline();}ds.Newline(reverse:true);});}public float GetHealth(IMyTerminalBlock block){IMySlimBlock slimblock=block.CubeGrid.GetCubeBlock(block.Position);if(slimblock
==null){return 1f;}float MaxIntegrity=slimblock.MaxIntegrity;float BuildIntegrity=slimblock.BuildIntegrity;float CurrentDamage=slimblock.CurrentDamage;return(BuildIntegrity-CurrentDamage)/MaxIntegrity;
}public void GetBlock(IMyTerminalBlock block){this.blocks.Add(block);}public void GotBLocks(){}public void Refresh(){if(this.blocks==null){return;}this.damaged.Clear();bool showOnHud=this.program.config.Enabled("healthOnHud");
foreach(var b in this.blocks){if(!Util.BlockValid(b)){continue;}if(this.ignoreHealth!=null&&this.ignoreHealth.IsMatch(b.CustomName)){continue;}var health=this.GetHealth(b);if(health!=1f){this.damaged[b.CustomName]
=Util.PctString(health);if(showOnHud&&!b.ShowOnHUD){this.shownOnHud[b.EntityId]=b;b.ShowOnHUD=true;}}}if(showOnHud){foreach(long key in this.shownOnHud.Keys.ToList()){IMyTerminalBlock block=this.shownOnHud[key];
if(block==null||!Util.BlockValid(block)){this.shownOnHud.Remove(key);}if(this.GetHealth(block)==1f){block.ShowOnHUD=false;this.shownOnHud.Remove(key);}}}this.status=$"{(this.damaged.Count == 0 ? "No damage" : "Damage")} detected";
}}public class PowerDetails:Runnable{public Program program;public Template template;public List<IMyTerminalBlock>powerProducerBlocks;public List<IMyTerminalBlock>jumpDriveBlocks;public List<IMyTerminalBlock>
consumers;public List<MyInventoryItem>items;public List<float>ioFloats;public List<Color>ioColours;public Dictionary<string,Color>ioLegendNames;public Dictionary<string,float>consumerDict;public float
batteryCurrent;public float batteryInput;public float batteryInputMax;public float batteryMax;public float batteryOutputDisabled;public float batteryOutputMax;public float batteryOutputMW;public float
batteryPotential;public float hEngineOutputDisabled;public float hEngineOutputMax;public float hEngineOutputMW;public float hEnginePotential;public float jumpCurrent;public float jumpMax;public float reactorOutputDisabled;
...
this.template.Register("power.chargeBar",this.ChargeBar);this.template.Register("power.batteryCurrent",()=>String.Format("{0:0.##}",this.batteryCurrent));this.template.Register("power.batteryInput",()
...
}public void ChargeBar(DrawingSurface ds,string text,DrawingSurface.Options options){if(this.batteries==0||this.batteryMax==0){return;}float pct=this.batteryCurrent/this.batteryMax;string colourName="dimred";
if(pct>0.85){colourName="dimgreen";}else if(pct>0.5){colourName="dimyellow";}options.pct=pct;options.fillColour=DrawingSurface.StringToColour(colourName);options.text=options.text??Util.PctString(pct);
ds.Bar(options);}public void BatteryBar(DrawingSurface ds,string text,DrawingSurface.Options options){if(this.batteries==0){return;}float io=this.batteryInput-this.batteryOutputMW;options.net=this.batteryCurrent;
float remainingMins=io==0f?0:(this.batteryMax-this.batteryCurrent)*60/Math.Abs(io);string pct=Util.PctString(this.batteryCurrent/this.batteryMax);if(this.batteryCurrent/this.batteryMax>=0.9999f){pct="100 %";
...
{this.Enable(productionBlock.block,true);}else{productionBlock.idleTime=-1;}return false;}public string IdleTime(ref ProductionBlock productionBlock){return Util.TimeFormat(this.Now()-productionBlock.idleTime);
}public string Status(IMyProductionBlock block){if(!block.IsFunctional){return"Broken";}if(block.IsProducing){return"Working";}if(block.IsQueueEmpty){return"Idle";}else{return"Blocked";}}public double
Now(){return DateTime.Now.Ticks/TimeSpan.TicksPerMillisecond;}public void Enable(IMyProductionBlock block,bool enabled){if(block.DefinitionDisplayNameText.ToString()!="Survival Kit"){block.Enabled=(bool)enabled;
}}}public class DrawingSurface{public class Options{public bool outline=false;public Color?bgColour=null;public Color?colour=null;public Color?fillColour=null;public Color?textColour=null;public float
height=0f;public float high=1f;public float low=1f;public float net=0f;public float pad=0.1f;public float pct=0f;public float scale=1f;public float size=0f;public float width=0f;public float?textPadding
=null;public List<Color>colours=new List<Color>();public List<float>values=new List<float>();public string text=null;public TextAlignment?align=null;public Dictionary<string,string>custom;public Options()
{this.custom=new Dictionary<string,string>();}}public Program program;public IMyTextSurface surface;public RectangleF viewport;public MySpriteDrawFrame frame;public Vector2 cursor;public Vector2 savedCursor;
public StringBuilder sb;public Vector2 charSizeInPx;public bool drawing;public Vector2 padding;public float width;public float height;public string name;public int ySpace;public Color colourGrey=new Color(40,
...
string FormatNumber(VRage.MyFixedPoint input,string fmt=null){if((int)input==0){return"0";}fmt=fmt??Util.GetFormatNumberStr(input);int n=Math.Max(0,(int)input);return n.ToString(fmt,GetCustomFormat());
}public static string TimeFormat(double ms,bool s=false){TimeSpan t=TimeSpan.FromMilliseconds(ms);if(t.Hours!=0){return String.Format("{0:D}h{1:D}m",t.Hours,t.Minutes);}if(t.Minutes!=0){return String.Format("{0:D}m",
t.Minutes);}return(s?String.Format("{0:D}s",t.Seconds):"< 1m");}public static string ToItemName(MyProductionItem i){string id=i.BlueprintId.ToString();if(id.Contains("IngotBasic")){return"Stone to ingot";
