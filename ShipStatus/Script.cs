...
alt);}public bool Enabled(string name){return this.settings.Get(name)=="true";}}public static string[]globalConfigs=new string[]{"airlock","production","cargo","power","health","gas","gasEnableFillPct",
"gasDisableFillPct","healthIgnore","airlockOpenTime","airlockAllDoors","airlockDoorMatch","airlockDoorExclude","healthOnHud","getAllGrids",};public bool ParseCustomData(){MyIniParseResult result;if(!ini.TryParse(Me.CustomData,
out result)){Echo($"Failed to parse config:\n{result}");return false;}if(Me.CustomData==""){Me.CustomData=customDataInit;}config.Clear();config.customData=Me.CustomData;strings.Clear();ini.GetSections(strings);
template.Reset();templates.Clear();config.Set("airlock","true");string themeConfig="";if(ini.ContainsSection("global")){string setting="";foreach(string cfg in globalConfigs){if(ini.Get("global",cfg).TryGetString(out
setting)){config.Set(cfg,setting);}}if(ini.Get("global","config").TryGetString(out setting)){config.Set("config",setting);themeConfig=$"{{config:{setting}}}\n";}}foreach(string outname in strings){if(outname
...
}if(tickCount%20==0){log.Clear();tickCount=0;}return false;}public void RunStateMachine(){if(stateMachine==null){return;}bool hasMoreSteps=stateMachine.MoveNext();if(hasMoreSteps){Runtime.UpdateFrequency
|=UpdateFrequency.Once;}else{stateMachine.Dispose();stateMachine=null;Runtime.UpdateFrequency&=~UpdateFrequency.Once;}}public IEnumerator<string>RunStuffOverTime(){string content;string outputName;while
(templates.Count>0){outputName=templates.Keys.Last();templates.Pop(templates.Keys.Last(),out content);Dictionary<string,bool>tokens;if(template.IsPrerendered(outputName,content)){tokens=template.templateVars[outputName];
}else{log.Append($"Adding or updating {outputName}\n");tokens=template.PreRender(outputName,content);}foreach(var kv in tokens){config.Set(kv.Key,config.Get(kv.Key,"true"));}yield return$"templates {outputName}";
if(templates.Count==0){powerDetails.Reset();cargoStatus.Reset();blockHealth.Reset();productionDetails.Reset();gasStatus.Reset();airlock.Reset();yield return"reset";RefetchBlocks();}yield return"updated";
}if(config.Enabled("power")){powerDetails.Refresh();yield return"powerDetails";}if(config.Enabled("cargo")){cargoStatus.Refresh();yield return"cargoStatus";}if(config.Enabled("health")){blockHealth.Refresh();
yield return"blockHealth";}if(config.Enabled("production")){productionDetails.Refresh();yield return"productionDetails";}if(config.Enabled("gas")){gasStatus.Refresh();yield return"gasStatus";}DrawingSurface.Options
themeOpts=null;string screenConfig=config.Get("config");if(screenConfig!=null){themeOpts=template.StringToOptions(screenConfig);}for(int j=0;j<drawables.Count;++j){var dw=drawables.ElementAt(j);if(themeOpts
!=null){template.ConfigureScreen(dw.Value,themeOpts);}template.Render(dw.Value);yield return$"render {dw.Key}";}config.Set("config",null);yield break;}GasStatus gasStatus;public class GasStatus{public
Program program;public Template template;public List<IMyGasTank>o2Tanks;public List<IMyGasTank>h2Tanks;public List<IMyGasGenerator>oxyGens;public Dictionary<string,double>tankMap;public double o2CurrentVolume;
public float o2MaxVolume;public double o2FillPct;public int o2TankCount;public double h2CurrentVolume;public float h2MaxVolume;public double h2FillPct;public int h2TankCount;public double enableFillPct;
public double disableFillPct;public GasStatus(Program program,Template template=null){this.program=program;this.template=template;this.o2Tanks=new List<IMyGasTank>();this.h2Tanks=new List<IMyGasTank>();
this.oxyGens=new List<IMyGasGenerator>();this.tankMap=new Dictionary<string,double>();this.Reset();}public void Reset(){this.Clear();if(this.program.config.Enabled("gas")){this.RegisterTemplateVars();
}}public void Clear(){this.ClearTotals();this.o2Tanks.Clear();this.h2Tanks.Clear();this.tankMap.Clear();}public void RegisterTemplateVars(){if(this.template==null){return;}this.template.Register("gas.o2CurrentVolume",
()=>$"{Util.FormatNumber(this.o2CurrentVolume)}");this.template.Register("gas.h2CurrentVolume",()=>$"{Util.FormatNumber(this.h2CurrentVolume)}");this.template.Register("gas.o2MaxVolume",()=>$"{Util.FormatNumber(this.o2MaxVolume)}");
this.template.Register("gas.h2MaxVolume",()=>$"{Util.FormatNumber(this.h2MaxVolume)}");this.template.Register("gas.o2FillPct",()=>Util.PctString(this.o2FillPct));this.template.Register("gas.h2FillPct",
()=>Util.PctString(this.h2FillPct));this.template.Register("gas.o2Tanks",(DrawingSurface ds,string text,DrawingSurface.Options options)=>{this.PrintTanks(this.o2Tanks,ds,options);});this.template.Register("gas.h2Tanks",
(DrawingSurface ds,string text,DrawingSurface.Options options)=>{this.PrintTanks(this.h2Tanks,ds,options);});this.template.Register("gas.generationEnabled",(DrawingSurface ds,string text,DrawingSurface.Options
options)=>{string message=options.custom.Get("txtDisabled")??"Gas generation off";if(this.GetGenerators()){message=options.custom.Get("txtEnabled")??"Gas generation on";}ds.Text(message,options);});this.template.Register("gas.generationEnabledExtended",
(DrawingSurface ds,string text,DrawingSurface.Options options)=>{this.GasExtendedMessage(ds,text,options);});this.template.Register("gas.o2Bar",(DrawingSurface ds,string text,DrawingSurface.Options options)
=>{this.GasBar((float)o2FillPct,ds,options);});this.template.Register("gas.h2Bar",(DrawingSurface ds,string text,DrawingSurface.Options options)=>{this.GasBar((float)h2FillPct,ds,options);});}public void
GetBlock(IMyTerminalBlock block){if(block is IMyGasTank){string name=block.DefinitionDisplayNameText.ToString();if(name.Contains("Oxygen Tank")){this.o2Tanks.Add((IMyGasTank)block);}else if(name.Contains("Hydrogen Tank"))
{this.h2Tanks.Add((IMyGasTank)block);}}else if(block is IMyGasGenerator){this.oxyGens.Add((IMyGasGenerator)block);}}public void GotBLocks(){this.enableFillPct=Util.ParseFloat(this.program.config.Get("gasEnableFillPct"),
-1f);this.disableFillPct=Util.ParseFloat(this.program.config.Get("gasDisableFillPct"),-1f);}public void Refresh(){this.ClearTotals();this.GetGasLevels();}public void ClearTotals(){this.o2CurrentVolume
...
ds,DrawingSurface.Options options){if(tanks.Count==0){ds.Text("");return;}foreach(IMyGasTank tank in tanks){double currentVolume=(tank.FilledRatio*tank.Capacity);ds.Text($@"{tank.CustomName}: {Util.FormatNumber(currentVolume)} / {Util.FormatNumber((double)tank.Capacity)} L ({Util.PctString(tank.FilledRatio)})",
...
program;public Template template;public System.Text.RegularExpressions.Regex ignoreHealth;public List<IMyTerminalBlock>blocks;public Dictionary<string,string>damaged;public Dictionary<long,IMyTerminalBlock>
shownOnHud;public string status;public BlockHealth(Program program,Template template){this.program=program;this.template=template;this.blocks=new List<IMyTerminalBlock>();this.damaged=new Dictionary<string,
string>();this.shownOnHud=new Dictionary<long,IMyTerminalBlock>();this.Reset();}public void Reset(){this.Clear();this.shownOnHud.Clear();if(this.program.config.Enabled("health")){this.RegisterTemplateVars();
...
(DrawingSurface ds,string text,DrawingSurface.Options options)=>{if(this.damaged.Count()==0){ds.Text("");return;}foreach(KeyValuePair<string,string>block in this.damaged){ds.Text($"{block.Key} [{block.Value}]").Newline();
...
{this.shownOnHud.Remove(key);}if(this.GetHealth(block)==1f){block.ShowOnHUD=false;this.shownOnHud.Remove(key);}}}this.status=$"{(this.damaged.Count == 0 ? "No damage" : "Damage")} detected";}}PowerDetails
powerDetails;public class PowerDetails{public Program program;public Template template;public List<IMyTerminalBlock>powerProducerBlocks;public List<IMyTerminalBlock>jumpDriveBlocks;public List<IMyTerminalBlock>
...
true);}}public ProductionDetails productionDetails;public class ProductionDetails{public Program program;public Template template;public List<MyProductionItem>productionItems;public List<ProductionBlock>
productionBlocks;public List<IMyProductionBlock>blocks;public Dictionary<ProductionBlock,string>blockStatus;public Dictionary<string,string>statusDotColour;public Dictionary<string,VRage.MyFixedPoint>
queueItems;public double productionCheckFreqMs=2*60*1000;public double productionOnWaitMs=5*1000;public double productionOutTimeMs=3*1000;public string productionIgnoreString="[x]";public string status;
public StringBuilder queueBuilder;public double idleTime=0;public double timeDisabled=0;public bool checking=false;public double lastCheck=DateTime.Now.Ticks/TimeSpan.TicksPerMillisecond;public char[]
splitNewline;public ProductionDetails(Program program,Template template){this.program=program;this.template=template;this.blocks=new List<IMyProductionBlock>();this.productionItems=new List<MyProductionItem>();
this.productionBlocks=new List<ProductionBlock>();this.blockStatus=new Dictionary<ProductionBlock,string>();this.queueItems=new Dictionary<string,VRage.MyFixedPoint>();this.statusDotColour=new Dictionary<string,
string>(){{"Broken","dimred"},{"Idle","dimgray"},{"Working","dimgreen"},{"Blocked","dimyellow"}};this.queueBuilder=new StringBuilder();this.splitNewline=new[]{'\n'};this.Reset();}public void Reset(){this.Clear();
if(this.program.config.Enabled("power")){this.RegisterTemplateVars();}}public void Clear(){this.blocks.Clear();this.productionItems.Clear();this.productionBlocks.Clear();this.blockStatus.Clear();this.queueItems.Clear();
this.idleTime=0;this.timeDisabled=0;this.checking=false;}public void RegisterTemplateVars(){if(this.template==null){return;}this.template.Register("production.status",()=>this.status);this.template.Register("production.blocks",
(DrawingSurface ds,string text,DrawingSurface.Options options)=>{bool first=true;if(this.blockStatus.Count()==0){ds.Text("");return;}foreach(KeyValuePair<ProductionBlock,string>blk in this.blockStatus)
{if(!first){ds.Newline();}string status=blk.Key.Status();string blockName=$" {blk.Key.block.CustomName}: {status} {(blk.Key.IsIdle() ? blk.Key.IdleTime() : "")}";Color?colour=DrawingSurface.StringToColour(this.statusDotColour.Get(status));
ds.TextCircle(colour,outline:false).Text(blockName);foreach(string str in blk.Value.Split(this.splitNewline,StringSplitOptions.RemoveEmptyEntries)){ds.Newline().Text(str);}first=false;}});}public void
GetBlock(IMyTerminalBlock block){if((block is IMyAssembler||block is IMyRefinery)&&!block.CustomName.Contains(this.productionIgnoreString)){this.productionBlocks.Add(new ProductionBlock(this.program,block
as IMyProductionBlock));}}public void GotBLocks(){this.productionBlocks=this.productionBlocks.OrderBy(b=>b.block.CustomName).ToList();}public void Refresh(){if(this.productionBlocks.Count==0){return;}
string itemName;bool allIdle=true;int assemblers=0;int refineries=0;this.blockStatus.Clear();this.status="";foreach(var block in this.productionBlocks){if(block==null||!Util.BlockValid(block.block)){continue;
}bool idle=block.IsIdle();if(block.block.DefinitionDisplayNameText.ToString()!="Survival Kit"){allIdle=allIdle&&idle;}if(idle){if(block.block is IMyAssembler){assemblers++;}else{refineries++;}}this.queueItems.Clear();
if(!block.IsIdle()){block.GetQueue(this.productionItems);foreach(MyProductionItem i in this.productionItems){itemName=Util.ToItemName(i);if(!this.queueItems.ContainsKey(itemName)){this.queueItems.Add(itemName,
i.Amount);}else{this.queueItems[itemName]=this.queueItems[itemName]+i.Amount;}}}this.queueBuilder.Clear();foreach(var kv in this.queueItems){this.queueBuilder.Append($"  {Util.FormatNumber(kv.Value)} x {kv.Key}\n");
...
productionItems){productionItems.Clear();block.GetQueue(productionItems);}public bool IsIdle(){string status=this.Status();if(status=="Idle"||status=="Broken"){this.idleTime=(this.idleTime==-1)?this.Now()
:this.idleTime;return true;}else if(status=="Blocked"&&!block.Enabled){block.Enabled=true;}this.idleTime=-1;return false;}public string IdleTime(){return Util.TimeFormat(this.Now()-this.idleTime);}public
string Status(){if(!this.block.IsFunctional){return"Broken";}else if(this.block.IsQueueEmpty&&!this.block.IsProducing){return"Idle";}else if(this.block.IsProducing){return"Working";}else if(!this.block.IsQueueEmpty
&&!this.block.IsProducing){return"Blocked";}return"???";}public double Now(){return DateTime.Now.Ticks/TimeSpan.TicksPerMillisecond;}}public class DrawingSurface{public class Options{public bool outline
=false;public Color?bgColour=null;public Color?colour=null;public Color?fillColour=null;public Color?textColour=null;public float height=0f;public float high=1f;public float low=1f;public float net=0f;
public float pad=0.1f;public float pct=0f;public float scale=1f;public float size=0f;public float width=0f;public float?textPadding=null;public List<Color>colours=new List<Color>();public List<float>values
=new List<float>();public string text=null;public TextAlignment?align=null;public Dictionary<string,string>custom;public Options(){this.custom=new Dictionary<string,string>();}}public Program program;
public IMyTextSurface surface;public RectangleF viewport;public MySpriteDrawFrame frame;public Vector2 cursor;public Vector2 savedCursor;public StringBuilder sb;public Vector2 charSizeInPx;public bool
drawing;public Vector2 padding;public float width;public float height;public string name;public int ySpace;public Color colourGrey=new Color(40,40,40);public bool mpSpriteSync=false;public StringBuilder
sizeBuilder;public static char[]underscoreSep={'_'};public static char[]commaSep={','};public static Dictionary<string,Color>stringToColour=new Dictionary<string,Color>(){{"black",Color.Black},{"blue",
Color.Blue},{"brown",Color.Brown},{"cyan",Color.Cyan},{"dimgray",Color.DimGray},{"dimgrey",Color.DimGray},{"gray",Color.Gray},{"green",Color.Green},{"orange",Color.Orange},{"pink",Color.Pink},{"purple",
Color.Purple},{"red",Color.Red},{"tan",Color.Tan},{"transparent",Color.Transparent},{"white",Color.White},{"yellow",Color.Yellow},{"dimgreen",Color.Darken(Color.Green,0.3)},{"dimyellow",Color.Darken(Color.Yellow,
